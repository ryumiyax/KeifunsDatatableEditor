...
def modify_nus3bank_template(game, template_name, audio_file, preview_point, output_file):
    ...
    template_data = template_data.replace(template_config['song_placeholder'].encode('utf-8'), os.path.splitext(os.path.basename(output_file_bytes))[0])
...
def convert_to_wav(input_file: str, temp_dir: str, target_peak_db: float = -1.5) -> str:
    ...
    return output_path
#
# def wav_to_hca_to_idsp_to_nus3bank(input_audio: str, out_file: str, preview_point: int, song_id: str):
#     vgaudio_cli_path = common.resource_path(os.path.join("bin", "VGAudioCli.exe"))
#     # with tempfile.TemporaryDirectory() as temp_dir:
#     hca_audio_file = os.path.join(os.getcwd(), f"{song_id}.hca")
#     subprocess.run([
#         vgaudio_cli_path,
#         "-i", input_audio,
#         "-o", hca_audio_file,
#         "--bitrate", "256000"
#     ], check=True)
#     idsp_audio_file = os.path.join(os.getcwd(), f"{song_id}.idsp")
#     subprocess.run([
#         vgaudio_cli_path,
#         "-i", hca_audio_file,
#         "-o", idsp_audio_file,
...
#     modify_nus3bank_template('nijiiro', template_name, idsp_audio_file, preview_point, out_file)
